...
%   \pause
%     \begin{equation*}
%       f(x_0) | y, \vectorx{x}, x_0 \sim GP(m^*, k^*) \\
%     \pause
%       \matrixx{K}_{f_y}
%       =
%       \scalemath{0.80}{
...
\frametitle{\secname}
  \begin{itemize}
    ...
    \only<3->{$\vectorx{k}_{\toblue{0}}^\top \tored{\matrixx{K}_{y}}^{-1} \tored{y}$}
     % posterior covariance
    \item \only<4->{$k_{*, \theta} (\toblue{x_0}, x_0^\prime) = k_\theta (\toblue{x_0}, \toblue{x_0}) -$}
    \only<5->{$\vectorx{k}_{\toblue{x_0}}^\top \tored{\matrixx{K}_{y}}^{-1} \vectorx{k}_{\toblue{0}}$}
    %
    \item<6-> La distribution de probabilités de la prédiction $y_0$ (connaissant les data d'entraînement) est connue !
    \item<6-> Accès à la prédiction moyenne, son incertitude (intervalle de confiance), quantiles, etc.
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{\secname}
  \begin{itemize}
    \item Exemple synthétique (très) simple:
    %
  \end{itemize}
\end{frame}
...
%             \overbrace{p(hypothèse)}^{\textcolor{red}{prior}}
%           }{
%             \underbrace{p(data)}_{\textcolor{red}{evidence}}
%           }
%         }
%         % formal way
%         \only<8>{
%           p(f_\theta | \vectorx{x}, y)
%           & =
%           \frac{
%             \overbrace{p(y | \vectorx{x}, f_\theta)}^{\textcolor{red}{likelihood}}
%             \overbrace{p(f_\theta)}^{\textcolor{red}{prior}}
%           }{
%             p(y | \vectorx{x})
%           } \\
%           & =
