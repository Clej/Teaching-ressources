      ...
      \begin{equation*}
        ...
        \only<9->{
          ...
          \frac{
            ...
            \overbrace{p(f_\theta)}^{\topurple{prior}}
          }{
            p(y | \vectorx{x})
          } 
          = GP(
...
\begin{frame}{\subsecname}
  ...
  \begin{itemize}
    \item <1-> $m_{*}, k_{*, \theta}$ ?
    \item \only<1->{$p(f_\theta | y, \vectorx{x}) = GP(m_{*}, k_{*, \theta})$}
     % posterior mean
    \item \only<2->{$m_*(\toblue{x_0}) = 0 +$}
    \only<3->{$\vectorx{k}_{\toblue{0}}^\top \tored{\matrixx{K}_{y}}^{-1} \tored{y}$}
     % posterior covariance
    \item \only<4->{$k_{*, \topurple{\theta}} (\toblue{x_0}, x_0^\prime) = k_\topurple{\theta} (\toblue{x_0}, \toblue{x_0}) -$}
...
\begin{frame}{\subsecname}
  ...
  \begin{itemize}
    ...
    \item $GP( m_{*}, k_{*, \theta} )$ est \coloredemph{approximée} par une distribution "variationnelle" $q_{\psi}$, proche, plus simple à calculer
    \pause
    \item (hyper-) paramètres appris par minimisation d'une "différence"\footnote{Ex: divergence KL.}  entre $q_{\psi}$ et $GP( m_{*}, k_{*, \theta} )$
    \pause
    \item Donc l'inférence devient un problème d'\coloredemph{optimisation}: gradient stochastique, differentiation automatique (PyTorch, Tensorflow, JAX), etc.
  \end{itemize}
\end{frame}

\begin{frame}{\subsecname}
  Méthodes d'\coloredemph{échantillonnage} (MCMC, importance sampling, MH, etc):
  \begin{itemize}
    \item Idée: remplacer $GP( m_{*}, k_{*, \theta} )$ par un échantillonneur peu coutêux
    \pause
    \item $m_{*}, k_{*, \theta}$ restent inconnus mais peuvent être estimés en échantillonnant (beaucoup) puis en moyennant (loi forte des grands nombres)
    \item $GP( m_{*}, k_{*, \theta} ) = p(f_{*, _theta}|\tored{y, x}) = \int p(f_* | f) p(f | y) df$
  \end{itemize}
\end{frame}
...
\begin{frame}{\subsecname}
  ...
  \begin{itemize}
    ...
    \item<3-> Exempe (classification): modéliser $\text{inv-logit} [ p(y=\text{Chat}) ]$ avec un $GP$
    \item<4-> Conséquence:
    \only<5>{
      \begin{equation*}
